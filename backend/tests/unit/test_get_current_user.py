...
async def test_valid_token_returns_profile():
    fake_profile = MagicMock(id=FAKE_UID, role="org_member")
    fake_db = MagicMock()
    ...
    with patch("app.core.dependencies.decode_supabase_token", new_callable=AsyncMock, return_value=FAKE_CLAIMS), \
