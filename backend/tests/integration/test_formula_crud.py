...
def template_version(db):
    ...
    db.add(t)
    v = TemplateVersion(
        id=_VER_ID, template_id=_TMPL_ID, version_number=1, schema_json='{"columns": []}', created_by=None
    )
    db.add(v)
    db.commit()
    return v
...
def test_create_formula(pif_admin_client, template_version):
    resp = pif_admin_client.post(
        f"/api/admin/template-versions/{_VER_ID}/formulas",
        json={
            "name": "TRIR",
            "target_column": "E",
            "formula_type": "column",
