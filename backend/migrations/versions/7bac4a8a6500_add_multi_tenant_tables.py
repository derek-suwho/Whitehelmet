...
def upgrade() -> None:
    ...
    op.alter_column(
        "organizations", "id", existing_type=sa.INTEGER(), type_=sa.String(length=36), existing_nullable=False
    )
    op.alter_column(
        "organizations", "parent_org_id", existing_type=sa.INTEGER(), type_=sa.String(length=36), existing_nullable=True
    )
...
def downgrade() -> None:
    ...
    op.add_column(
        "uploaded_files", sa.Column("folder_name", sa.VARCHAR(length=500), autoincrement=False, nullable=True)
    )
    op.add_column(
        "organizations", sa.Column("external_id", sa.VARCHAR(length=255), autoincrement=False, nullable=False)
    )
    ...
    op.alter_column(
        "organizations", "parent_org_id", existing_type=sa.String(length=36), type_=sa.INTEGER(), existing_nullable=True
    )
    op.alter_column(
        "organizations", "id", existing_type=sa.String(length=36), type_=sa.INTEGER(), existing_nullable=False
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["org_id"], ["organizations.id"], name="org_memberships_org_id_fkey", ondelete="CASCADE"
        ),
